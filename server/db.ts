...
export async function markManyAttendance(input: { date: string; records: Array<{ studentId: string; status: "present" | "absent" | "late" }> }) {
  for (const record of input.records) await markAttendance({ ...record, date: input.date });
}
