...
import { getStoredContent, listAttendanceHistory, listStudents, lookupStudent, markAttendance, markManyAttendance, saveStoredContent, saveStudent } from "./db";
...
export const appRouter = router({
  ...
  attendance: router({
    ...
    markMany: adminOnly.input(z.object({ date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/), records: z.array(z.object({ studentId: z.string().trim().min(1).max(64), status: z.enum(["present", "absent", "late"]) })).min(1) })).mutation(async ({ input }) => { await markManyAttendance(input); return { success: true } as const; }),
  }),
});
